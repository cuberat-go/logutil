...
// file in the ~/log/ directory (when the log level is ERROR or higher) and to
// stderr (when the log level is INFO or higher). The error log file is
// created on the first write. The log file name is generated based on the
// program name, current timestamp, and process ID.
package main
